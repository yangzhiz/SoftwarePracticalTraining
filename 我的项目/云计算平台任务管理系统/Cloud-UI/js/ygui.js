...
(function($) {
    $.fn.jqSelect = function(o) {
        ...
        select = $(this).attr('class','combox').css({'display':'none'}),
        ...
        setElem = function() {
            ...
            select.parent().css({ "display": "inline","float":"left","margin-right":"2px" }).height(25);
...
(function($) {
     $.fn.reJqSelect = function(options){
		...
		var ul=$(this).nextAll("ul.ylDropDownUl");
		 ...
		 for(i=0;i<len;i++)
		 {
			ul.append('<li style="background-color: transparent; border: 1px solid rgb(255, 255, 255);"><div>'+$(opt[i]).text()+'</div></li>');
		 }
		 var lis=$("li",ul);
		 ul.css({'height':'auto'})
		 ...
		 lis.click(function(event){
				event.stopPropagation();
                var index = lis.index($(this));
				var sel=$(this).parent().parent().find('select').show().css({'position':'absolute','left':'-100px'});//ie6 隐藏bug
				try{
